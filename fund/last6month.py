...
# from fund.model import Last6month, db
...
from fund.models import Last6month, DayGrowRate, Last1month, Last6month, Last1week, Last1year
...
@app.route('/show')
def show():
    datas = Last6month.query.order_by(Last6month.date.desc()).all()
    # print(res)
...
def insert():
    # user = Last6month.query.filter_by(id=id).first()
    if request.method == "POST":
        ...
        data = {
            ...
            'type': form.get('type'),
            'name': form.get('name'),
            'last6month': form.get('last6month'),
            'date': form.get('date'),
        }
        f = Last6month(**data)
        db.session.add(f)
...
def update(id):
    ...
    if request.method == "POST":
        # print('form', request.form.to_dict())
        form = request.form.to_dict()
        Last6month.query.filter_by(id=id).update(
            {'last6month': form.get('price'), 'date': form.get('date'), 'type': form.get('type')})
        db.session.commit()
        return redirect(url_for('show'))
...
def delete(id):
    ...
    if request.method == "POST":
        # print('form', request.form.to_dict())
        # form = request.form.to_dict()
        Last6month.query.filter_by(id=id).delete()
        db.session.commit()
        # return '删除成功'
        return redirect(url_for('show'))
...
def batch_delete(date):
    user = Last6month.query.filter_by(date=date).first()

    if request.method == "POST":
        # print('form', request.form.to_dict())
        # form = request.form.to_dict()
        Last6month.query.filter_by(date=date).delete()
        db.session.commit()
        # return '删除成功'
        return redirect(url_for('show'))
...
def filter_condition(kind, start_time, end_time: str):
    return Last6month.query.filter_by(type=kind).filter(Last6month.date >= start_time,
                                                        Last6month.date <= end_time).order_by('date').all()


def get_x_y_datas_2022():
    # x_data = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"]
    # x_data = ["2022/8/{}".format(i + 1) for i in range(21, 31)]
    # date_instances = Last6month.query.filter_by(type='gp').order_by('date').all()
...
def get_x_y_datas():
    ...
    x_data = dates
    # 股票型
    gp_instances = Last6month.query.filter_by(type='gp').order_by('date').all()
    gp_datas = [i.last6month for i in gp_instances]
    # 债券型
    zq_instances = Last6month.query.filter_by(type='zq').order_by('date').all()
    zq_datas = [i.last6month for i in zq_instances]
    # 指数型
    zs_instances = Last6month.query.filter_by(type='zs').order_by('date').all()
    zs_datas = [i.last6month for i in zs_instances]
    # qdii型
    qdii_instances = Last6month.query.filter_by(type='qdii').order_by('date').all()
    qdii_datas = [i.last6month for i in qdii_instances]
    # fof型
    fof_instances = Last6month.query.filter_by(type='fof').order_by('date').all()
    fof_datas = [i.last6month for i in fof_instances]
    # 混合型
    hh_instances = Last6month.query.filter_by(type='hh').order_by('date').all()
    hh_datas = [i.last6month for i in hh_instances]
    # print('hh_datas', hh_datas)
    return x_data, gp_datas, zq_datas, zs_datas, qdii_datas, fof_datas, hh_datas
