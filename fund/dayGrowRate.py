...
# from fund.model import DayGrowRate, db
...
from fund.models import DayGrowRate
...
def insert():
    # user = DayGrowRate.query.filter_by(id=id).first()
    if request.method == "POST":
        ...
        data = {
            ...
            'type': form.get('type'),
            'name': form.get('name'),
            'dayGrowRate': form.get('dayGrowRate'),
            'date': form.get('date'),
        }
        f = DayGrowRate(**data)
        db.session.add(f)
...
def update(id):
    ...
    if request.method == "POST":
        # print('form', request.form.to_dict())
        form = request.form.to_dict()
        DayGrowRate.query.filter_by(id=id).update(
            {'dayGrowRate': form.get('price'), 'date': form.get('date'), 'type': form.get('type')})
        db.session.commit()
        return redirect(url_for('show'))
...
def delete(id):
    ...
    if request.method == "POST":
        # print('form', request.form.to_dict())
        # form = request.form.to_dict()
        DayGrowRate.query.filter_by(id=id).delete()
        db.session.commit()
        # return '删除成功'
        return redirect(url_for('show'))
...
def batch_delete(date):
    user = DayGrowRate.query.filter_by(date=date).first()

    if request.method == "POST":
        # print('form', request.form.to_dict())
        # form = request.form.to_dict()
        DayGrowRate.query.filter_by(date=date).delete()
        db.session.commit()
        # return '删除成功'
        return redirect(url_for('show'))
...
def filter_condition(kind, start_time, end_time: str):
    return DayGrowRate.query.filter_by(type=kind).filter(DayGrowRate.date >= start_time,
                                                         DayGrowRate.date <= end_time).order_by('date').all()


def get_x_y_datas_2022():
    # x_data = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"]
    # x_data = ["2022/8/{}".format(i + 1) for i in range(21, 31)]
    # date_instances = DayGrowRate.query.filter_by(type='gp').order_by('date').all()
...
def get_x_y_datas():
    ...
    x_data = dates
    # 股票型
    gp_instances = DayGrowRate.query.filter_by(type='gp').order_by('date').all()
    gp_datas = [i.dayGrowRate for i in gp_instances]
    # 债券型
    zq_instances = DayGrowRate.query.filter_by(type='zq').order_by('date').all()
    zq_datas = [i.dayGrowRate for i in zq_instances]
    # 指数型
    zs_instances = DayGrowRate.query.filter_by(type='zs').order_by('date').all()
    zs_datas = [i.dayGrowRate for i in zs_instances]
    # qdii型
    qdii_instances = DayGrowRate.query.filter_by(type='qdii').order_by('date').all()
    qdii_datas = [i.dayGrowRate for i in qdii_instances]
    # fof型
    fof_instances = DayGrowRate.query.filter_by(type='fof').order_by('date').all()
    fof_datas = [i.dayGrowRate for i in fof_instances]
    # 混合型
    hh_instances = DayGrowRate.query.filter_by(type='hh').order_by('date').all()
    hh_datas = [i.dayGrowRate for i in hh_instances]
    # print('hh_datas', hh_datas)
    return x_data, gp_datas, zq_datas, zs_datas, qdii_datas, fof_datas, hh_datas
